-- =============================
-- Complete Authentication Fix for Supabase Auth + Profiles
-- =============================
...
-- Let's create a backup and then drop and recreate it with the correct schema
...
-- Optional: Insert existing users into profiles table (if any)
-- Uncomment the following lines if you have existing users without profiles:
/*
INSERT INTO profiles (id, full_name, email, avatar_url, role, status)
SELECT id, raw_user_meta_data->>'full_name', email, raw_user_meta_data->>'avatar_url', 'user', 'active'
FROM auth.users
ON CONFLICT (id) DO NOTHING;
*/

-- Test the setup by checking if the profiles table exists and has the correct structure:
